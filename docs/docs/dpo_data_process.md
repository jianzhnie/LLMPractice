...
| `--cache_dir`          | `str`  | `<HF_CACHE_DIR>`                 | Hugging Face 缓存目录。                                 |
...
- `chosen`: **(必需)** 已经过 `tokenizer.apply_chat_template` 格式化的"**正确**"或"**更优**"的助手回答。
- `rejected`: **(必需)** 已经过 `tokenizer.apply_chat_template` 格式化的"**错误**"或"**较差**"的助手回答。

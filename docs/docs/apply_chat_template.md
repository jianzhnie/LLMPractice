...
| 参数                  | 类型   | 默认值                         | 描述                                                         |
| --------------------- | ------ | ------------------------------ | ------------------------------------------------------------ |
| `--model-dir`         | `str`  | `/home/<user>/llmtuner/hfhub/models/` | 本地模型存储的基础目录。此值可通过环境变量 `HF_MODEL_DIR` 覆盖。 |
...
例如，对于 Model: Qwen2.5-7B，Prompt Type: openr1_prompt，模型将使用 OpenR1 的 System Prompt 构建 Message，然后使用 Qwen2.5-7B 模型的聊天模板进行格式化，最终输出如下：
...
```text
<|im_start|>system
You are a helpful AI Assistant that provides well-reasoned and detailed responses.<|im_end|>
```

...
```text
10.42.24.130:/mnt/nfs_share  /mnt/nfs_clientside  nfs  defaults  0  0
```
...
1. **防火墙策略**：如果你开启了防火墙，请在服务端节点上放行来自 `10.42.24.0/24` 的流量。
2. **性能优化**：如果你在进行高并发读写（如深度学习训练），可以在客户端挂载参数中添加 `rsize=32768,wsize=32768` 以提升吞吐量。
3. **单点故障**：NFS 服务端一旦宕机，所有客户端的挂载点都会变得不可用（"Stale file handle"）。如果这是生产环境，后期可以考虑使用 **Keepalived + NFS** 做高可用。
...
1. **在 130 节点**：`touch /data/nfs/test_file.txt`
2. **在 145 节点（或其他任意节点）**：`ls /data/nfs/test_file.txt`

如果能看到该文件，说明路径已完全统一。

### 4. 进阶：符号链接 (Symbolic Link)
...
这样，无论实际挂载点在哪，你的应用程序只需要访问 `/projects/data` 即可。

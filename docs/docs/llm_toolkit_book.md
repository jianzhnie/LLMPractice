...
第一个命令将默认索引源设置为阿里源。第二个命令将删除所有额外的索引源。
...
## 十六、Wandb 同步 TensorBoard 日志

如果你有本地 TensorBoard 日志（如 `.tfevents` 文件）并希望同步到 Weights & Biases (W&B)，可以按照以下步骤操作：
...
这样，TensorBoard 生成的 `.tfevents` 日志会自动同步到 W&B。
